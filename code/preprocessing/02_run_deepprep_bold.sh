...
#
# Usage:
#   bash code/preprocessing/02_run_deepprep_bold.sh              # all subjects, task=rest
#   bash code/preprocessing/02_run_deepprep_bold.sh sub-01 rest  # one subject, one task
# =============================================================================
set -euo pipefail
...
BIDS_DIR="$PROJ_DIR/data/raw/fmri_cannabis_dataset"
...
if [ ! -d "$BIDS_DIR" ]; then
  ...
  echo "     $PROJ_DIR/data/raw/fmri_cannabis_dataset/"
  echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
  ...
  echo ""
  echo "   Next: python3 code/analysis/04_functional_analysis.py"
else
