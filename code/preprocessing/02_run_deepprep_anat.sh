...
#
# Usage:
#   bash code/preprocessing/02_run_deepprep_anat.sh --pilot           # all pilot subjects, both sessions
#   bash code/preprocessing/02_run_deepprep_anat.sh --pilot BL        # pilot subjects, baseline only
#   bash code/preprocessing/02_run_deepprep_anat.sh sub-101 FU        # single subject, follow-up only
#   bash code/preprocessing/02_run_deepprep_anat.sh sub-101           # single subject, both sessions
# =============================================================================
set -euo pipefail

DEEPPREP_IMAGE="pbfslab/deepprep:25.1.0"
PROJ_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BIDS_DIR="$PROJ_DIR/data/raw/ds000174"
OUTPUT_DIR="$PROJ_DIR/data/derivatives/deepprep"
...
if [ ! -d "$BIDS_DIR" ]; then
  ...
  echo "   Run: bash code/preprocessing/01_download_data.sh"
  exit 1
fi
...
if [ "${1:-}" = "--pilot" ]; then
  PILOT_FILE="$PROJ_DIR/data/processed/pilot_subjects.txt"
  if [ ! -f "$PILOT_FILE" ]; then
    echo "❌ pilot_subjects.txt not found."
    echo "   Run first: python3 code/analysis/03_group_participants.py"
    exit 1
  fi
...
echo ""
echo "Next: python3 code/analysis/04_structural_analysis.py"
